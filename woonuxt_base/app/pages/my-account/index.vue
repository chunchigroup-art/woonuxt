...
// 💡 核心修复：防止因异步数据未就绪导致页面无限转圈
// 改为：只要 viewer 的状态已经判定完成（无论是已登录还是未登录），或者已经拿到 viewer 实例，就不再显示全局 Loader
const isAuthChecking = ref(true);
const showLoader = computed(() => {
  // 如果已经判定出 viewer 存在，或者判定过程已结束，就不再强制转圈
  if (viewer.value !== undefined) {
    return false;
  }
  return isAuthChecking.value;
});

// 在组件挂载后，设置一个兜底计时器，防止页面死锁
onMounted(() => {
  setTimeout(() => {
    isAuthChecking.value = false;
  }, 1500); // 1.5秒后强制放行，优先渲染出登录表单
});
    ...
    <template v-else>
      ...
      <div v-else class="flex flex-col items-start justify-between w-full gap-8 mb-24 lg:flex-row">
        ...
        <aside class="w-full lg:w-72 lg:sticky lg:top-20 shrink-0">
          ...
          <nav class="p-3 mb-6 bg-white border border-gray-100 rounded-lg shadow-xs">
            <NuxtLink to="/my-account?tab=orders" class="nav-link" :class="{ active: activeTab == 'orders' }">
              ...
              <span class="font-semibold text-gray-900">Order History</span>
            </NuxtLink>
        ...
        <main class="flex-1 w-full min-w-0">
          ...
          <PersonalInformation v-else-if="activeTab === 'personal-info'" />
